...
public:
	...
	std::shared_ptr<TileMap> constructTileMap(const std::string tileset, int numTileTypes, Vector2d inPosition, Vector2d* inTileSize, Vector2d* inTileSetSize, bool centerMap, bool randTexture = true);
...
private:
	...
	int colourShadeTiles(std::shared_ptr<TileMap> map, int r, int g, int b, int a, unsigned int minTileExtentX, unsigned int minTileExtentY, unsigned int maxTileExtentX, unsigned int maxTileExtentY, unsigned int shadeSteps, bool randomise = false, Vector2d* areaPos = nullptr);
};